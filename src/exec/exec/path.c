...
/*   Updated: 2025/09/03 13:28:29 by lserodon         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
char	**get_path_array(t_exec_data *exec_data)
{
	...
	if (!path)
		ft_fatal_error(exec_data, "missing PATH", 127, &free_exec);
	path_array = ft_split(path, ':');
	free(path);
	if (!path_array)
		ft_fatal_error(exec_data, "ft_split", 1, &free_exec);
	return (path_array);
}
...
char	*build_cmd_path(t_exec_data *exec_data, char *dir, int i)
{
	...
	if (!tmp_path)
		ft_fatal_error(exec_data, "ft_strjoin", 1, &free_exec);
...
char	*check_access(t_exec_data *exec_data, char **array, char *path)
{
	if (access(path, F_OK) == 0)
	{
		...
		else
		{
			free_array(array);
			free(path);
			ft_fatal_error(exec_data, "permission denied", 126, &free_exec);
		}
	}
	return (NULL);
}
